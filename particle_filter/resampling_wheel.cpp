...
int main()
{
    ...
    // find the largest value in w[n]
    double w_max = max(w, n); // a convenient function
    /*for (int i = 0; i < n; i++)
    {
        w_max = w_max > w[i] ? w_max : w[i];
    }*/
    
    // Initialize a resampled set of particles
    Robot p3[n];
    
    // main loop for resampling wheel
    for (int i = 0; i < n; i++) 
    {
        // Note1: need to multiply uncertainty
        beta += 2 * w_max * gen_real_random();
        while (w[index] < beta)
        {
            beta -= w[index];
            index++;
            // Note2: need to consider the index overflow cases
            // index %= n;
            index = mod(index, n);
        }
