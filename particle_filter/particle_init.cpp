...
int main()
{
    ...
    // Loop over the set of particles, with specific noise parameters
    double Forward_Noise = 0.05, Turn_Noise = 0.05, Sense_Noise = 5.0;
    for (int i = 0; i < n; i++) {
        double x_position = gen_real_random() * 100;
        double y_position = gen_real_random() * 100;
        double orient     = gen_real_random() * M_PI * 2;
        
        p[i].set(x_position, y_position, orient);
        p[i].set_noise(Forward_Noise, Turn_Noise, Sense_Noise);

        // Print its pose on a single line
        cout << p[i].show_pose() << ' ';
    }
    
    // Simulate motion of each particle
